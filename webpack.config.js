...
module.exports = {
  ...
  entry: {
  	main: __dirname + '/resources/js/entry.js',
  	css: __dirname + '/resources/js/css.js',
		component: __dirname + '/resources/js/app.js'
  },
  //文件出口
  output: {
  	path: __dirname + '/dist',//文件出口的路径zoz
  ...
  module: {
    loaders: [
      ...
      //如果不是使用配置文件则可以这样写require('style!css!./style.css')
      {
      	...
      	//loaders: ["style","css","postcss"] --默认的加载顺序是从右往左
			...
			{
				test: /\.html$/,
				loader: 'html-loader',//导入时html代码为字符串
    ...
    new htmlWebpackPlugin({
	   	template: 'webpack.ejs',//在进行插件配置的时候，需要将你选择作为template的文件改为ejs。html-webpack-plugin会将ejs的模板文件解析为html并输出。
		...
		new htmlWebpackPlugin({
	   	template: 'index.ejs',//在进行插件配置的时候，需要将你选择作为template的文件改为ejs。html-webpack-plugin会将ejs的模板文件解析为html并输出。
	   	filename: 'index.html',
	   	inject: 'body',
  ...
  //     alias: {
  //         'jquery': __dirname + 'resources/js/jquery-vendor.js',
  //     }
  // },
